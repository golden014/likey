...
const EditableField = ({data, func, name} : {data : string, func: any, name:any}) => {
    ...
    return (
        <div className='w-full flex flex-row'>
            ...
            {(edit == "true") &&
                <button onClick={action} className='w-10 h-full bg-green-200 hover:bg-green-300 active:bg-green-400 rounded-r-md'>
                    <FontAwesomeIcon icon={faCheck} />
                </button>
            }
            {(edit == "loading") &&
                <button className='w-10 h-full bg-gray-300 rounded-r-md flex items-center justify-center'>
